...
const StepSelectProvider = ({ value, onSelect }) => {
  const providers = [
    { id: "gmail", name: "Gmail", type: "image", icon: googleLogo },
        ...
        {providers.map((provider) => (
          <button
            ...
            className={`flex flex-col items-center justify-center gap-2 border rounded-lg p-6 hover:bg-gray-50 transition ${
              value === provider.id ? "border-black shadow-md" : "border-gray-200"
            }`}
          >
            {provider.type === "image" ? (
...
export default StepSelectProvider;
